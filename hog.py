...
def roll_dice(num_rolls, dice=six_sided):
    ...
    # BEGIN PROBLEM 1
    "*** YOUR CODE HERE ***"
    #问题就在这里！ 当第一次掷出1时，你直接返回1，后面的第2次和第3次骰子根本就没掷！
    i = 1
    total = 0
    flag = True
    while i <= num_rolls :
...
def boar_brawl(player_score, opponent_score):
    ...
    # BEGIN PROBLEM 2
    "*** YOUR CODE HERE ***"
    the_ones_digit_of_player = player_score % 10
    the_tens_digit_of_opponent = (opponent_score // 10) % 10
    if the_ones_digit_of_player == the_tens_digit_of_opponent:
        return 1
    elif the_tens_digit_of_opponent > the_ones_digit_of_player:
        return 3 * (the_tens_digit_of_opponent - the_ones_digit_of_player)
    else:
        return 3 * (the_ones_digit_of_player - the_tens_digit_of_opponent)
    # END PROBLEM 2
...
def take_turn(num_rolls, player_score, opponent_score, dice=six_sided):
    ...
    # BEGIN PROBLEM 3
    "*** YOUR CODE HERE ***"
    #这里的 dice=six_sided 是默认参数：
    # 如果调用时不提供 dice 参数，就自动使用 six_sided
    # 如果调用时提供了 dice 参数，就使用提供的值
    if num_rolls == 0:
...
def play(strategy0, strategy1, update, score0=0, score1=0, dice=six_sided, goal=GOAL):
    ...
    #用传入的【函数参数】而非自己参照个例 函数名是可以作为参数的
    while score0 < goal and score1 < goal:
        if update == sus_update:
            if who == 0:
                score0 = sus_update(strategy0(score0, score1), score0,score1,dice)
            else:
                score1 = sus_update(strategy1(score1, score0), score1,score0,dice)
        else:
            if who == 0:
...
def max_scoring_num_rolls(dice=six_sided, times_called=1000):
    ...
    for num in range(1,11):
        result = make_averaged(roll_dice,times_called)(num,dice)
        if result > max_total:
            max_total = result
            max_num = num
    return max_num
...
def sus_strategy(score, opponent_score, threshold=11, num_rolls=6):
    ...
    Suss Fuss rules."""
    # BEGIN PROBLEM 11
    num = num_factors(score + boar_brawl(score,opponent_score))
    if num == 3 or num == 4:
        final_score = sus_points(score + boar_brawl(score,opponent_score))
    else:
        final_score = score + boar_brawl(score,opponent_score)
